...
// 🧭 WANDERLUST: scoop up every mine and scatter it across the still-hidden
// cells (never under one you've already revealed), then recount every number
// on the board. your carefully placed flags are now beautiful lies.
function relocateMines() {
  const candidates = [];
  for (const cells of grid) {
    for (const cell of cells) {
      cell.mine = false;
      if (!cell.revealed) candidates.push(cell);
    }
  }
  for (let i = 0; i < config.mines && candidates.length > 0; i++) {
    const index = Math.floor(Math.random() * candidates.length);
    const [cell] = candidates.splice(index, 1);
    cell.mine = true;
  }
  for (const cells of grid) {
    for (const cell of cells) {
      cell.adjacentMines = neighbours(cell).filter((n) => n.mine).length;
      if (cell.revealed) paintCell(cell);
    }
  }
  // little cursed shudder so you SEE the ground shift under your feet
...
function onRightClick(cell) {
  ...
  paintCell(cell);
  updateCounters();
  // 🧭 WANDERLUST: planting a flag spooks the mines into wandering off.
  if (minesPlaced && cell.flagged) relocateMines();
}
